             ...
             double &UTMNorthing, double &UTMEasting, QString &UTMZone)
{
    ...
    UTMZone = QString("%1%2").arg(ZoneNumber,2,10,QChar('0')).arg(UTMLetterDesignator(Lat));
    ...
    UTMNorthing = static_cast<double>(k0*(M+N*tan(LatRad)*(A*A/2+(5-T+9*C+4*C*C)*A*A*A*A/24
              ...
              double& Lat,  double& Long )
{
    ...
    ZoneNumber = strtoul(UTMZone.toUtf8().constData(), &ZoneLetter, 10);
    if((*ZoneLetter - 'N') >= 0)
    {
    }
    else
